    ...
    fn: (argv, context) => {
        ...
        searchWiki.then(function(body) {
            ...
            if (searchResults[1].length > 0) {
                ...
                var getWikiArticle = request({
                    uri: parseUrl,
                    method: 'GET'
                });


                getWikiArticle.then(function(body) {
                    ...
                    if (article.parse.images === undefined || article.parse.images.length == 0) {
                        ...
                        var getWikiImage = request({
                            uri: wikiImageUrl + article.parse.images[0],
                            method: 'GET'
                        });

                        getWikiImage.then(function(response) {
                            ...
                            console.log('Couldn\'t fetch article image.');
                            console.log(error);
                            context.msg.channel.sendEmbed(embed);
                        });
                    ...
                    console.log('Couldn\'t fetch wiki article.');
                    console.log(error);
                    context.msg.channel.sendMessage('Could not fetch wiki article. Please try again later.');
                });
            ...
            console.log('Couldn\'t search wiki.');
            console.log(error);
            context.msg.channel.sendMessage('Could not fetch wiki article. Please try again later.');
        });
